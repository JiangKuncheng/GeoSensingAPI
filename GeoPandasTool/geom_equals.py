...
def geom_equals(geojson_names: Union[str, List[str]], other_geojson_name: str) -> Union[bool, Dict[str, bool]]:
    """
    判断一个或多个 GeoJSON 文件中的几何对象是否与另一个 GeoJSON 文件中的几何对象完全相同

    参数:
        geojson_names (Union[str, List[str]]):
            - 单个 GeoJSON 文件名（不含路径和扩展名）
            - 或多个文件名组成的列表
        other_geojson_name (str): 目标 GeoJSON 文件名，判断主几何是否与该几何完全相同
    ...
    for name in names:
        ...
        try:
            ...
            for geometry in gseries:
                result.append(any(geometry.equals(other_geometry) for other_geometry in other_geometries))

            # 返回整体结果（任一几何对象与目标几何完全相同）
