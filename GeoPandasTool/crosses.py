...
def crosses(geojson_names: Union[str, List[str]], other_geojson_name: str) -> Union[bool, Dict[str, bool]]:
    """
    ...
    参数:
        geojson_names (Union[str, List[str]]):
            ...
            - 或多个文件名组成的列表
        other_geojson_name (str): 目标 GeoJSON 文件名，判断主几何是否与该几何相交
    ...
    for name in names:
        ...
        try:
            ...
            for geometry in gseries:
                result.append(any(geometry.crosses(other_geometry) for other_geometry in other_geometries))

            # 返回整体结果（任一几何对象与目标几何相交）
