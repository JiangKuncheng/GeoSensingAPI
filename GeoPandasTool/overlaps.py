...
def overlaps(geojson_str, other_geojson_str):
    """
    判断 Overpass API 获取的 GeoJSON 数据中的几何对象是否与目标几何对象部分重叠。

    参数:
        geojson_str (str): 主 GeoJSON 字符串
        other_geojson_str (str): 目标 GeoJSON 字符串，判断主几何是否与其部分重叠

    返回:
        list: 每个 geometry 是否与目标 geometry 部分重叠的布尔值
    """
    # 解析 GeoJSON
