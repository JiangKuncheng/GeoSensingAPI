...
def geom_equals_exact(geojson_names: Union[str, List[str]], other_geojson_name: str, tolerance: float = 1e-9) -> Union[bool, Dict[str, bool]]:
    """
    判断一个或多个 GeoJSON 文件中的几何对象是否与另一个 GeoJSON 文件中的几何对象完全相同（在精度容差范围内）

    参数:
        geojson_names (Union[str, List[str]]):
            - 单个 GeoJSON 文件名（不含路径和扩展名）
            - 或多个文件名组成的列表
        other_geojson_name (str): 目标 GeoJSON 文件名，判断主几何是否与该几何完全相同（在容差范围内）
        tolerance (float): 容差范围，用于判断几何是否完全相同，默认为 1e-9
