...
def geom_equals_exact(geojson_str, other_geojson_str, tolerance=1e-9):
    """
    判断 Overpass API 获取的 GeoJSON 中的几何对象是否与另一个几何对象完全相同（在精度容差范围内）

    参数:
        geojson_str (str): 主 GeoJSON 字符串
        other_geojson_str (str): 目标 GeoJSON 字符串，判断主几何是否与该几何完全相同（在容差范围内）
        tolerance (float): 容差范围，用于判断几何是否完全相同，默认为 1e-9

    返回:
        list: 每个 geometry 是否与目标 geometry 完全相同（在容差范围内）的布尔值
    """
    # 解析 geojson
