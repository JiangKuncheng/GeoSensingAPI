...
def bounds(*geojson_strs, multiInvocation=False, times=1):
    """
    计算一个或多个 Overpass API 导出的 GeoJSON 的边界包围盒坐标
    ...
    返回:
        list[pandas.DataFrame] | pandas.DataFrame:
            如果 multiInvocation 为 True，返回每个 GeoJSON 的 bounds 列表；
            否则返回单个 GeoJSON 的 bounds。
    """
    if multiInvocation:
        ...
        for geojson_str in geojson_strs:
            ...
            bounds_list.append(gseries.bounds)
        return bounds_list
    else:
